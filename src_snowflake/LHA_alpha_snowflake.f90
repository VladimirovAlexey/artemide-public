...
!         end do
    ...
    ERROR STOP ErrorString('Effective LambdaQCD, is too high...',moduleName)

    write(*,'("AlphaS prepared with Effective LambdaQCD = ",F10.6)') LambdaEFF
    LambdaEFF=max(LambdaEFF+0.1d0,0.4d0)

    write(*,'(A)') color("----- Alpha_s from "//trim(name)//" loaded. ",c_yellow)&
...
if(Q<Qmin) then !!! logarith log-extrapolation
    if(Q<LambdaEFF) ERROR STOP ErrorString('Q is smaller than Effective LambdaQCD',moduleName)
...
end module LHA_alpha_snowflake
