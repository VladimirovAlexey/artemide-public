...
!!!------------------------ THIS IS A PART OF artemide v1. adapted for v.3 -------------------------------
!!!-----------   !!!IT IS NOT USED FOR ANY COMPUTATIONS EXCEPT FOR COMPARISON WITH OTHER CODES!!!   ------
!!!-------------------------------------------------------------------------------------------------------
...
function TMDR_R_type1(b,muf,zetaf,mui,zetai,mu0,f)
  ...
  !!! original version from artemide 1
  !TMDR_R_type1=EXP(IntegralG1(muf,mui,zetaf,f)-(IntegralG2(mui,mu0,f)+DNP(mu0,b,f))*Log(zetaf/zetai))
  
  TMDR_R_type1=EXP(IntegralG1(muf,mui,zetaf,f)-CS_kernel(mui,b,f)*Log(zetaf/zetai))
...
!!! The evolution from point (muf,zetaf) -> (zeta_mui) via scale mu0
 function TMDR_Rzeta_type1(b,muf,zetaf,mui,mu0,f)
  real(dp)::TMDR_Rzeta_type1
  real(dp),intent(in):: b,muf,zetaf,mui,mu0
  integer,intent(in)::f
  ...
  zetaP=zetaMUpert(mui,b,f) !zetaNP(mui,b,f)
  
  !!! original version from artemide 1
  !TMDR_Rzeta_type1=EXP(IntegralG1(muf,mui,zetaf,f)-(IntegralG2(mui,mu0,f)+DNP(mu0,b,f))*Log(zetaf/zetaP))

  TMDR_Rzeta_type1=EXP(IntegralG1(muf,mui,zetaf,f)-CS_kernel(mui,b,f)*Log(zetaf/zetaP))
