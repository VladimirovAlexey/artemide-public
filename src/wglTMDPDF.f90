...
function wglTMDPDF_IsInitialized()
    logical::wglTMDPDF_IsInitialized
    wglTMDPDF_IsInitialized=started
end function wglTMDPDF_IsInitialized
...
subroutine wglTMDPDF_SetPDFreplica_tw3(rep,hadron)
    ...
    call wglTMDPDF_OPE_tw3_SetPDFreplica(rep,hadron)
...
subroutine wglTMDPDF_SetScaleVariation_tw3(c4_in)
    real(dp),intent(in)::c4_in
    call wglTMDPDF_OPE_tw3_SetScaleVariation(c4_in)
...
function wglTMDPDF_CurrentLambdaNP()
...
!!!!!!! the function that actually returns the wglTMDPDF!
...
!!!!!!!! the function that actually returns the wglTMDPDF evolved to (mu,zeta) value
...
function wglTMDPDF_TMM_X(x,mu,hadron)
    real(dp)::wglTMDPDF_TMM_X(-5:5)
    real(dp),intent(in):: x,mu
    integer,intent(in)::hadron
