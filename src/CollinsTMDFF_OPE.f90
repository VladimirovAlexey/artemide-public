...
character (len=16),parameter :: moduleName="CollinsTMDFF_OPE"
...
!!--------------------------------------Public interface-----------------------------------------
public::CollinsTMDFF_OPE_IsInitialized,CollinsTMDFF_OPE_Initialize
public::CollinsTMDFF_OPE_tw3_convolution,CollinsTMDFF_OPE_tw3_resetGrid,CollinsTMDFF_OPE_tw3_testGrid
public::CollinsTMDFF_OPE_tw3_SetFFreplica,CollinsTMDFF_OPE_tw3_SetScaleVariation
...
function CollinsTMDFF_OPE_IsInitialized()
    logical::CollinsTMDFF_OPE_IsInitialized
    CollinsTMDFF_OPE_IsInitialized=started
end function CollinsTMDFF_OPE_IsInitialized
...
subroutine CollinsTMDFF_OPE_Initialize(file,prefix)
    ...
    !!! Model initialisation is called from the CollinsTMDFF-module
    gridReady=.false.
...
!     end if
    ...
    if(outputLevel>0) write(*,*) color('----- arTeMiDe.CollinsTMDFF_OPE '//trim(version)//': .... initialized',c_green)
    if(outputLevel>1) write(*,*) ' '

end subroutine CollinsTMDFF_OPE_Initialize
...
function CollinsTMDFF_OPE_tw3_convolution(x,b,h,addGluon)
    real(dp),dimension(-5:5)::CollinsTMDFF_OPE_tw3_convolution
    ...
    !!!! test for boundaries is done in CollinsTMDFF_lowScale5 (on the enty to this procedure)

    !!!! case NA
    if(orderMainTW3==-50) then
        if(gluon) then
            CollinsTMDFF_OPE_tw3_convolution=1._dp
        else
            CollinsTMDFF_OPE_tw3_convolution=(/1._dp,1._dp,1._dp,1._dp,1._dp,0._dp,1._dp,1._dp,1._dp,1._dp,1._dp/)
        end if
        return
    end if
...
subroutine CollinsTMDFF_OPE_tw3_resetGrid()
    !!!!! not implemented yet
end subroutine CollinsTMDFF_OPE_tw3_resetGrid

!!! This subroutine force reconstruction of the grid (if griding is ON)
subroutine CollinsTMDFF_OPE_tw3_testGrid()
    !!!!! not implemented yet
end subroutine CollinsTMDFF_OPE_tw3_testGrid

!! call QCDinput to change the PDF replica number
!! unset the grid, since it should be recalculated fro different PDF replica.
subroutine CollinsTMDFF_OPE_tw3_SetFFreplica(rep,hadron)
...
subroutine CollinsTMDFF_OPE_tw3_SetScaleVariation(c4_in)
    ...
    !!!! not implemented yet
end subroutine CollinsTMDFF_OPE_tw3_SetScaleVariation

end module CollinsTMDFF_OPE
