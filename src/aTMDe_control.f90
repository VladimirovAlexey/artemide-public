...
contains
  ...
  if(.not.include_uTMDPDF) then
    if(outputLevel>0) &
      write(*,*) ErrorString('attempt to set NP-parameters for uTMDPDF, while uTMDPDF module is not included in the current setup',&
      moduleName)
    if(outputLevel>0) write(*,*) color('NOTHING IS DONE',c_red)
    return
  end if
  ...
  if(rep<1) then			! case of technical replicas
    ...
    elseif(rep<=numR) then				! main pull of replicas
      call MoveTO(51,'*R   ')
      do i=1,numR
       read(51,*) ParametersTOread
       repString=ParametersTOread(2:lenArray)
